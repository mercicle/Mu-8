...
function generateLinesFile(nicolasPDBFile){
	...
	colorsAndVertices += "var vertices = [\n";
	...
	for (var i=0; i<nseg; i++) {
		...
		colorsAndVertices += String(at0.x) + ", " + String(at0.y) + ", " + String(at0.z) + ", " + String(at1.x) + ", " + String(at1.y) + ", " + String(at1.z) + ",\n";
	}
	
	colorsAndVertices += "0];\n";
	

	colorsAndVertices += "var colors = [\n";
	...
	for (var i=0; i < nseg; i++)
	{
		...
		colorsAndVertices += String(r0) + ", " + String(g0) + ", " + String(b0) + ", " + String(atomArray[indices[i][0]].aminonum) + ", " + String(r1) + ", " + String(g1) + ", " + String(b1) + ", " + String(atomArray[indices[i][1]].aminonum)+ ", \n";
	...
	var initBuffers = "";
	initBuffers += "var linesVertexPositionBuffer;\n";
	initBuffers += "var linesVertexColorBuffer;\n";
	initBuffers += "var quadPosBuffer;\n";
	
	initBuffers += "function initBuffers() {\n";
	initBuffers += "    linesVertexPositionBuffer = gl.createBuffer();\n";
	initBuffers += "    gl.bindBuffer(gl.ARRAY_BUFFER, linesVertexPositionBuffer);\n";
	initBuffers += "    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vertices), gl.STATIC_DRAW);\n";
	initBuffers += "	linesVertexPositionBuffer.numItems = " + String(nseg*2) + " ;\n";
	initBuffers += "    linesVertexPositionBuffer.itemSize = 3;\n";
	
	initBuffers += "linesVertexColorBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, linesVertexColorBuffer);\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);\n";
	initBuffers += "linesVertexColorBuffer.itemSize = 4;\n";
	initBuffers += "linesVertexColorBuffer.numItems = " + String(nseg*2) + ";\n";
	
	
	
	initBuffers += "linesTangentBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, linesTangentBuffer);\n";
	initBuffers += "var tangents = new Array();\n";
	initBuffers += "for (i=0; i<linesVertexColorBuffer.numItems; i++) {\n";
	initBuffers += "    for (j=0; j<6; j++) {\n";
	initBuffers += "        tangents[i*6+j] = vertices[i*6+j%3]-vertices[i*6+j%3+3];\n";
	initBuffers += "    }\n";
	initBuffers += "    var n1 = Math.sqrt(tangents[i*6+0]*tangents[i*6+0]+tangents[i*6+1]*tangents[i*6+1]+tangents[i*6+2]*tangents[i*6+2]);\n";
	initBuffers += "    var n2 = Math.sqrt(tangents[i*6+3]*tangents[i*6+3]+tangents[i*6+4]*tangents[i*6+4]+tangents[i*6+5]*tangents[i*6+5]);\n";
	initBuffers += "    if (n1 == 0 || isNaN(n1)) n1=0.0001;\n";
	initBuffers += "    if (n2 == 0 || isNaN(n2)) n2=0.0001;\n";
	initBuffers += "    tangents[i*6+0]/=n1; tangents[i*6+1]/=n1; tangents[i*6+2]/=n1;\n";
	initBuffers += "    tangents[i*6+3]/=n2; tangents[i*6+4]/=n2; tangents[i*6+5]/=n2;\n";
	initBuffers += "}\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(tangents), gl.STATIC_DRAW);\n";
	initBuffers += "linesTangentBuffer.numItems = " + String(nseg*2) + " ;\n";
	initBuffers += "linesTangentBuffer.itemSize = 3;\n";
	initBuffers += "\n";
	initBuffers += "\n";
	initBuffers += "// generate cylinders instead of lines\n";
	initBuffers += "\n";
	initBuffers += "var Ncyl = 6;\n";
	initBuffers += "var R = 0.3;\n";
	initBuffers += "var vtxCyl = new Array();\n";
	initBuffers += "var normalsCyl = new Array();\n";
	initBuffers += "var colorsCyl = new Array();\n";
	initBuffers += "var trianglesCyl = new Array();\n";
	initBuffers += "for (i=0; i<" + String(nseg) + "; i++) {\n";
	initBuffers += "    for (j=0; j<Ncyl; j++) {\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+0] = colors[i*8+0];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+1] = colors[i*8+1];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+2] = colors[i*8+2];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+3] = colors[i*8+3];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+0+Ncyl*4] = colors[i*8+4];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+1+Ncyl*4] = colors[i*8+5];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+2+Ncyl*4] = colors[i*8+6];\n";
	initBuffers += "        colorsCyl[i*Ncyl*2*4+j*4+3+Ncyl*4] = colors[i*8+7];\n";
	initBuffers += "\n";
	initBuffers += "        var oX = 0.;\n";
	initBuffers += "        var oY = tangents[i*6+2];\n";
	initBuffers += "        var oZ = -tangents[i*6+1]; //orthogonal vector   \n";
	initBuffers += "        var n = Math.sqrt(oX*oX+oY*oY+oZ*oZ);\n";
	initBuffers += "        if (n == 0 || isNaN(n)) n = 0.0001;\n";
	initBuffers += "        oX/=n;\n";
	initBuffers += "        oY/=n;\n";
	initBuffers += "        oZ/=n;\n";
	initBuffers += "        var bX = tangents[i*6+1]*oZ-tangents[i*6+2]*oY; //binormal vector\n";
	initBuffers += "        var bY = tangents[i*6+2]*oX-tangents[i*6+0]*oZ;\n";
	initBuffers += "        var bZ = tangents[i*6+0]*oY-tangents[i*6+1]*oX;   \n";
	initBuffers += "        var theta = j*3.1416*2./Ncyl;\n";
	initBuffers += "        var ctheta = Math.cos(theta)*R;\n";
	initBuffers += "        var stheta = Math.sin(theta)*R;\n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+0] = vertices[i*6+0] + oX*ctheta + bX*stheta;\n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+1] = vertices[i*6+1] + oY*ctheta + bY*stheta; \n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+2] = vertices[i*6+2] + oZ*ctheta + bZ*stheta;\n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+0] = oX*ctheta + bX*stheta;\n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+1] = oY*ctheta + bY*stheta;\n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+2] = oZ*ctheta + bZ*stheta;\n";
	initBuffers += "\n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+0+Ncyl*3] = vertices[i*6+3] + oX*ctheta + bX*stheta;\n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+1+Ncyl*3] = vertices[i*6+4] + oY*ctheta + bY*stheta;\n";
	initBuffers += "        vtxCyl[i*Ncyl*6+j*3+2+Ncyl*3] = vertices[i*6+5] + oZ*ctheta + bZ*stheta; \n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+0+Ncyl*3] = oX*ctheta + bX*stheta;\n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+1+Ncyl*3] = oY*ctheta + bY*stheta;\n";
	initBuffers += "        normalsCyl[i*Ncyl*6+j*3+2+Ncyl*3] = oZ*ctheta + bZ*stheta;   \n";
	initBuffers += "\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+0] = i*Ncyl*2+j;\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+1] = i*Ncyl*2+(j+1)%Ncyl;\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+2] = i*Ncyl*2+j+Ncyl;\n";
	initBuffers += "\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+3] = i*Ncyl*2+(j+1)%Ncyl;\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+4] = i*Ncyl*2+j+Ncyl;\n";
	initBuffers += "        trianglesCyl[i*Ncyl*6+j*6+5] = i*Ncyl*2+(j+1)%Ncyl+Ncyl;   \n";
	initBuffers += "    }\n";
	initBuffers += "}\n";
	initBuffers += "\n";
	initBuffers += "vtxCylBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, vtxCylBuffer);\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vtxCyl), gl.STATIC_DRAW);\n";
	initBuffers += "vtxCylBuffer.numItems = " + String(nseg*2) + "*Ncyl;\n";
	initBuffers += "vtxCylBuffer.itemSize = 3;\n";
	initBuffers += "\n";
	initBuffers += "normalsCylBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, normalsCylBuffer);\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(normalsCyl), gl.STATIC_DRAW);\n";
	initBuffers += "normalsCylBuffer.numItems = " + String(nseg*2) + "*Ncyl;\n";
	initBuffers += "normalsCylBuffer.itemSize = 3;	\n";
	initBuffers += "\n";
	initBuffers += "colCylBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, colCylBuffer);\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colorsCyl), gl.STATIC_DRAW);\n";
	initBuffers += "colCylBuffer.numItems = " + String(nseg*2) + "*Ncyl;\n";
	initBuffers += "colCylBuffer.itemSize = 4;\n";
	initBuffers += "\n";
	initBuffers += "triCylBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, triCylBuffer);\n";
	initBuffers += "gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(trianglesCyl), gl.STATIC_DRAW);\n";
	initBuffers += "triCylBuffer.numItems = " + String(nseg*2) + "*Ncyl*3;\n";
	initBuffers += "triCylBuffer.itemSize = 3;\n";
	initBuffers += "\n";
	initBuffers += "\n";
	initBuffers += "var vtxQuad = new Array();\n";
	initBuffers += "vtxQuad[0]=-1.;\n";
	initBuffers += "vtxQuad[1]=-1.;\n";
	initBuffers += "vtxQuad[2]=0.;\n";
	initBuffers += "vtxQuad[3]=-1.;\n";
	initBuffers += "vtxQuad[4]=1.;\n";
	initBuffers += "vtxQuad[5]=0.;\n";
	initBuffers += "vtxQuad[6]=1.;\n";
	initBuffers += "vtxQuad[7]=1.;\n";
	initBuffers += "vtxQuad[8]=0.;\n";
	initBuffers += "vtxQuad[9]=-1.;\n";
	initBuffers += "vtxQuad[10]=-1.;\n";
	initBuffers += "vtxQuad[11]=0.;\n";
	initBuffers += "vtxQuad[12]=1.;\n";
	initBuffers += "vtxQuad[13]=1.;\n";
	initBuffers += "vtxQuad[14]=0.;\n";
	initBuffers += "vtxQuad[15]=1.;\n";
	initBuffers += "vtxQuad[16]=-1.;\n";
	initBuffers += "vtxQuad[17]=0.;\n";
	initBuffers += "\n";
	initBuffers += "\n";
	initBuffers += "quadPosBuffer = gl.createBuffer();\n";
	initBuffers += "gl.bindBuffer(gl.ARRAY_BUFFER, quadPosBuffer);\n";
	initBuffers += "gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vtxQuad), gl.STATIC_DRAW);\n";
	initBuffers += "quadPosBuffer.numItems = 6;\n";
	initBuffers += "quadPosBuffer.itemSize = 3;	\n";
	initBuffers += "\n";
	initBuffers += "}\n";
	
	return [colorsAndVertices, initBuffers];
